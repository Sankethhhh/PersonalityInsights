...
from big5_data import questions120, reversal, facets_questions
import json
import hashlib
...
def save_to_json(data):
    ...
    try:
        with open("big5_database.json", "r") as file:
    ...
    with open("big5_database.json", "w") as file:
        json.dump(existing_data, file, indent=4)

def main():
    st.set_page_config(page_title="Elevated Leaders - BIG5", layout="centered")

    #  -----HEADER SECTION-----
    st.subheader("Elevated Leaders")
    st.title("BIG FIVE")
    st.write("The Big Five personality model, also known as the Five-Factor Model (FFM), identifies five broad dimensions that encompass human personality traits: Openness, Conscientiousness, Extraversion, Agreeableness, and Neuroticism (OCEAN).")
    st.write("Each dimension represents a spectrum, with individuals falling somewhere along each trait. Openness reflects a person's creativity and willingness to embrace new experiences, Conscientiousness measures organization and reliability, Extraversion gauges sociability, Agreeableness assesses interpersonal cooperation, and Neuroticism evaluates emotional stability. The Big Five model is widely used in psychology to understand and describe personality variations.")  
    st.write("---")
